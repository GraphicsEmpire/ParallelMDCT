...
inline void mat4FromQuat(const vec4f& quat, MAT44& result)
{
...
{
    vec3f p0(mat.e[0][0], mat.e[0][1], mat.e[0][2]);
    vec3f p1(mat.e[1][0], mat.e[1][1], mat.e[1][2]);
    vec3f p2(mat.e[2][0], mat.e[2][1], mat.e[2][2]);

    vec3f c12 = vcross3f(p1, p2);
...
inline void mat4Translate(const vec3f& t, MAT44& mat)
{
	mat.e[3][0] = t.x;
	mat.e[3][1] = t.y;
	mat.e[3][2] = t.z;
...
inline void mat4Scale(const vec3f& s, MAT44& mat)
{
	MAT44 work;
	work.e[0][0] = s.x;
	work.e[1][1] = s.y;
...
inline vec3f mat4Transform(const MAT44& input, const vec3f& v)
{
	vec3f row0 = vec3f(input.e[0][0], input.e[1][0], input.e[2][0]);
	vec3f row1 = vec3f(input.e[0][1], input.e[1][1], input.e[2][1]);
	vec3f row2 = vec3f(input.e[0][2], input.e[1][2], input.e[2][2]);
	vec3f col3 = vec3f(input.e[3][0], input.e[3][1], input.e[3][2]);

	vec3f t = vec3f(vdot3f(row0, v), vdot3f(row1, v), vdot3f(row2, v));
	t = vadd3f(t, col3);
	return t;
}
