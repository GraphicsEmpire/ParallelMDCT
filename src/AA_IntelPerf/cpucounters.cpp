...
/* FOR TESTING PURPOSES ONLY */
    ...
    while (0 != fgets(buffer, 1024, f_cpuinfo))
    {
        ...
        if (strncmp(buffer, "physical id", sizeof("physical id") - 1) == 0)
        {
            ...
            num_sockets = (std::max)(num_sockets, entry.socket + 1);
            continue;
        }
...
PCM::ErrorCode PCM::program(PCM::ProgramMode mode_, void * parameter_)
{
    ...
    if (!numInstancesSemaphore) return PCM::UnknownError;
    LONG prevValue = 0;
    if (!ReleaseSemaphore(numInstancesSemaphore, 1, &prevValue)) return PCM::UnknownError;
    ...
    {
        if (EACCES == errno)
            std::cout << "PCM Error, do not have permissions to open semaphores in /dev/shm/" << std::endl;
    ...
    else
    {
        ...
        {
            coreEventDesc[0].event_number = MEM_LOAD_UOPS_MISC_RETIRED_LLC_MISS_EVTNR;
            coreEventDesc[0].umask_value = MEM_LOAD_UOPS_MISC_RETIRED_LLC_MISS_UMASK;
