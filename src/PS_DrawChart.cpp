...
void CDrawChart::draw(int plotMode)
{
    for(size_t i=0; i<m_series.size(); i++)
    {
        glVertexPointer(2, GL_FLOAT, 0, &m_series[i].vData[0]);
        glEnableClientState(GL_VERTEX_ARRAY);

        glColorPointer(3, GL_FLOAT, 0, &m_series[i].vColor[0]);
        glEnableClientState(GL_COLOR_ARRAY);

        if(plotMode & PLOT_LINE_STRIP)
            glDrawArrays(GL_LINE_STRIP, 0, m_series[i].vData.size() / 2);
        if(plotMode & PLOT_POINTS)
            glDrawArrays(GL_POINTS, 0, m_series[i].vData.size() / 2);
